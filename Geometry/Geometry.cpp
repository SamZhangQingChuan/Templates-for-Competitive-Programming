...
const flt eps = 1e-8, pi = std::acos(-1.0);
...
struct Point {
    ...
    [[nodiscard]] T dot(const Point &o) const { return x * o.x + y * o.y; }

    [[nodiscard]] T det(const Point &o) const { return x * o.y - y * o.x; }

    [[nodiscard]] T norm2() const { return x * x + y * y; }

    [[nodiscard]] flt norm() const { return hypot(x, y); }

    [[nodiscard]] flt ang() const { return atan2(y, x); }

    [[nodiscard]] Point perp() const { return Point(-y, x); } // rotate 90 degrees
    [[nodiscard]] Point unit() const { return *this / norm(); }

    [[nodiscard]] Point trunc(flt k) const { return unit() * k; }

    // counter-clockwise rotate a rad
    [[nodiscard]] Point rot(flt a) const {
        return Point(x * cos(a) - y * sin(a), x * sin(a) + y * cos(a));
    }

    // counter-clockwise rotate using cos/sin
    [[nodiscard]] Point rot(flt cosa, flt sina) const {
